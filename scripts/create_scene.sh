...
cp pkg/scenes/scene_template.go "pkg/scenes/scene_${some_name}.go"

# Replace all occurrences of "Template" with "SomeName" in the new file
sed -i "s/Template/$SomeName/g" "pkg/scenes/scene_${some_name}.go"

echo "Processed template for $SomeName and saved to pkg/scenes/scene_${some_name}.go"
