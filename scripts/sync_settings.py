...
def replace_fallback_settings(fallback_values, source):
    function_pattern = r"func FallbackSettings\(\) \{(.*?)\}"
    function_match = re.search(function_pattern, source, re.DOTALL)
    if not function_match:
        raise ValueError("FallbackSettings function not found")

    fallback_function = "func FallbackSettings() {\n\tsettings = &GameSettings{\n"
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        input_file = "./pkg/settings/settings.go"
